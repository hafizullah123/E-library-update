...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["coverImage"]) && $_FILES["coverImage"]["error"] == UPLOAD_ERR_OK) {
        ...
        if (!move_uploaded_file($_FILES["coverImage"]["tmp_name"], $coverImagePath)) {
            echo "<div class='alert alert-danger' role='alert'>Cover image upload failed. Please try again.</div>";
            exit;
        }
    } elseif (isset($_FILES["coverImage"]["error"]) && $_FILES["coverImage"]["error"] != UPLOAD_ERR_NO_FILE) {
        echo "<div class='alert alert-danger' role='alert'>Error uploading cover image: " . $_FILES["coverImage"]["error"] . "</div>";
        exit;
    }
    ...
    if (isset($_FILES["pdf"]) && $_FILES["pdf"]["error"] == UPLOAD_ERR_OK) {
        ...
        if (!move_uploaded_file($_FILES["pdf"]["tmp_name"], $pdfPath)) {
            echo "<div class='alert alert-danger' role='alert'>PDF upload failed. Please try again.</div>";
            exit;
        }
    } elseif (isset($_FILES["pdf"]["error"]) && $_FILES["pdf"]["error"] != UPLOAD_ERR_NO_FILE) {
        echo "<div class='alert alert-danger' role='alert'>Error uploading PDF: " . $_FILES["pdf"]["error"] . "</div>";
        exit;
    }
